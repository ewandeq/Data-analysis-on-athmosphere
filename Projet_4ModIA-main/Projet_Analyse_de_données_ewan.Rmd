...
```{r}
Kmax <- 15
Iintra_hierarchical <- numeric(Kmax - 1)

for (k in 2:Kmax) {
  # Calculer le clustering hiérarchique avec k clusters
  cutree_clusters <- cutree(res_hc, k)
  str(cutree_clusters)
  # Calculer l'inertie intraclasse
  total_withinss <- 0
  for (i in 1:k) {
...
df_intra_hierarchical <- data.frame(K = 2:15, Iintra = Iintra_hierarchical)

# Utiliser ggplot pour tracer l'évolution de l'inertie intraclasse
library(ggplot2)

ggplot(df_intra_hierarchical, aes(x = K, y = Iintra)) +
  geom_line() +
  geom_point(size = 1) +
